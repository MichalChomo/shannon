...
**Options:** `CONFIG=<file>` (YAML config), `OUTPUT=<path>` (default: `./audit-logs/`), `WORKSPACE=<name>` (named workspace; auto-resumes if exists), `PIPELINE_TESTING=true` (minimal prompts, 10s retries), `REBUILD=true` (force Docker rebuild), `ROUTER=true` (multi-model routing via [Codex-router](https://github.com/musistudio/Codex-router))
...
- `src/ai/Codex-executor.ts` — Codex Agent SDK integration with retry logic
...
- **SDK Integration** — Uses `@anthropic-ai/Codex-agent-sdk` with `maxTurns: 10_000` and `bypassPermissions` mode. Playwright MCP for browser automation, TOTP generation via MCP tool. Login flow template at `prompts/shared/login-instructions.txt` supports form, SSO, API, and basic auth
...
**Core Logic:** `src/session-manager.ts`, `src/ai/Codex-executor.ts`, `src/config-parser.ts`, `src/services/`, `src/audit/`
